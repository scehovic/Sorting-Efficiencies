...
public abstract class Sorter
{
  ...
  public void sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort) throws NullPointerException, IllegalArgumentException{
    // TODO
	  CountingComparator<String> countCom = new CountingComparator<String>(comp);
	 
	  PrintWriter p = null;
	  long startTime = System.currentTimeMillis(); 
	  WordList copy = null;
	  while(totalWordsSorted < totalToSort) { //iterate through until condition met, update variables every cycle
		  copy = toSort.clone();
		  sort(copy, countCom);
		  totalWordsSorted += copy.length();
		  totalSortingTime += System.currentTimeMillis();
		  totalComparisons += countCom.getCount();	  
		  countCom.reset();
	  ...
	  try {
		  p = new PrintWriter(new File("C:/Users/ryans/Desktop/output100")); //file path & file name 
	  }
	  catch (Exception e) {
		  e.printStackTrace();
	  }
	  for(int i = copy.length()-1; i >=0; i--) { //prints out the sorted version the file
		  p.println(copy.get(i));
		  p.flush();
	  }
  }
